...
export function LocationPicker({ label, value, allowGps, onChange }: { label: string; value: RouteLocation | null; allowGps?: boolean; onChange: (value: RouteLocation) => void }) {
  ...
  return <section className="rounded-xl border border-slate-200 bg-slate-50 p-3">
    ...
    <div className="relative"><input className="min-h-12 w-full rounded-xl border border-slate-200 bg-white px-4 text-base font-bold text-slate-950 caret-road-500 outline-none placeholder:text-slate-400 focus:border-road-500" value={text} onChange={(e) => setText(e.target.value)} placeholder={label === "Откуда" ? "Казань" : "Москва"} />
      {suggestions.length ? <div className="absolute z-30 mt-1 w-full overflow-hidden rounded-xl border bg-white text-slate-950 shadow-xl">{suggestions.map((item) => <button type="button" key={`${item.lat}:${item.lon}`} className="block w-full border-b px-4 py-3 text-left text-slate-950 hover:bg-road-50" onClick={() => select(item)}><b>{item.title || item.name}</b><span className="block text-sm text-slate-500">{item.address}</span></button>)}</div> : null}
    </div>
