...
export function RouteForm({ from, to, isRouteMode, isBuildingRoute, onFromChange, onToChange, onSwap, onBuildRoute }: { from: RouteLocation | null; to: RouteLocation | null; isRouteMode: boolean; isBuildingRoute: boolean; onFromChange: (value: RouteLocation) => void; onToChange: (value: RouteLocation) => void; onSwap: () => void; onBuildRoute: () => void }) {
  ...
  async function selectRecent(name: string) { const result = (await searchGeo(name)).results[0]; if (result) onToChange({ ...result, type: "address", text: result.address }); }
  return <section className="grid gap-3 rounded-[26px] border border-white/[.08] bg-[#10151d] p-4 text-white shadow-2xl">
    ...
    <div><div className="mb-2 text-xs font-black uppercase tracking-wider text-[#8a94a6]">Последние</div><div className="flex gap-2 overflow-x-auto">{["Москва", "Уфа", "Адлер"].map((city) => <button key={city} type="button" onClick={() => void selectRecent(city)} className="min-h-11 rounded-full bg-white/[.08] px-4 text-sm font-black active:scale-90">{city}</button>)}</div></div>
