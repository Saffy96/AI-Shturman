...
export function FiltersPanel({ filters, onChange }: { filters: StationFilters; onChange: (filters: StationFilters) => void }) {
  return (
    <section className="filters-panel" aria-label="Дополнительные фильтры">
      ...
      {filters.availability === "withFuel" ? <p className="filter-explanation">АЗС, где топливо есть или заканчивается, включая станции с очередью. Без топлива и неподтверждённые данные скрыты. Очередь можно исключить отдельно.</p> : null}
    </section>
  );
}
