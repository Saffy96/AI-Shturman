...
export function FiltersPanel({ filters, onChange }: { filters: StationFilters; onChange: (filters: StationFilters) => void }) {
  return (
    <section className="filters-panel" aria-label="Дополнительные фильтры">
      <Chip icon={<Check size={15} />} active={filters.availability === "withFuel"} onClick={() => onChange({ ...filters, availability: filters.availability === "withFuel" ? "all" : "withFuel" })}>Точно в наличии</Chip>
      ...
      {filters.availability === "withFuel" ? <p className="filter-explanation">АЗС с подтверждённым топливом, включая станции с очередью. Мало топлива, нет топлива и неподтверждённые данные скрыты. Очередь можно исключить отдельно.</p> : null}
    </section>
  );
}
