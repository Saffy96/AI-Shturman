...
import { Check, Fuel, Route, Sparkles, Users } from "lucide-react";
...
export function FiltersPanel({ filters, onChange }: { filters: StationFilters; onChange: (filters: StationFilters) => void }) {
  return (
    <section className="filters-panel" aria-label="Дополнительные фильтры">
      <Chip icon={<Fuel size={15} />} active={filters.availability === "withSelectedFuel"} onClick={() => onChange({ ...filters, availability: filters.availability === "withSelectedFuel" ? "all" : "withSelectedFuel" })}>Выбранное топливо</Chip>
