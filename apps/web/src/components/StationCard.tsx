...
export function StationCard({ station }: { station: FuelStation }) {
  ...
  return <article id={`station-${station.id}`} className="group scroll-mt-40 rounded-[28px] border border-white/80 bg-white/85 p-4 shadow-[0_18px_55px_rgba(15,23,42,.10)] backdrop-blur-xl transition hover:-translate-y-0.5 hover:shadow-xl">
    ...
    <div className="mt-3 flex flex-wrap gap-2"><span className="rounded-full bg-slate-950 px-3 py-1.5 text-xs font-black text-white">{station.statusLabel}</span>{station.fuels.length ? station.fuels.map((fuel) => <span key={fuel} className="rounded-full bg-emerald-100 px-3 py-1.5 text-xs font-black text-emerald-900">{fuel} ✓</span>) : <span className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-black text-slate-600">Марки топлива неизвестны</span>}<span className={`rounded-full px-3 py-1.5 text-xs font-black ${station.hasQueue ? "bg-orange-100 text-orange-900" : "bg-sky-100 text-sky-900"}`}>{station.hasQueue ? "Очередь" : "Без очереди"}</span><span className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-black text-slate-600">{station.freshnessLabel}</span></div>
    <div className="mt-4 rounded-2xl bg-slate-950 p-3 text-sm font-bold text-slate-200">{station.recommendation}<div className="mt-2 text-[10px] uppercase tracking-wider text-slate-400">Данные водителей ГдеБЕНЗ · уверенность {station.confidence == null ? "—" : `${Math.round(station.confidence * 100)}%`}</div><div className="mt-1 text-[10px] text-slate-500">Пользовательские отметки, не официальные данные сети АЗС.</div></div>
